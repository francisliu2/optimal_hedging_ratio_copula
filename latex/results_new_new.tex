...
We analyse the hedge portfolios in this section. \natp{\em [Careful,
  it's hedge portfolios, not hedged portfolios. The asset is hedged,
  not the portfolio.]}
Tables~\ref{tab:var_rh} to table~\ref{tab:ERM_rh} summarise the statistics
of daily returns of hedged portfolios.
\natp{\em [Is there a way to aggregate the tables? This would be
  useful, as otherwise the reader keeps scrolling back and
  forth. Perhaps we can remove skewness and kurtosis, then plot in one
  large table in landscape mode (with a lot of columns).]}
...
%\include{_tables/ES5_rh}
%\include{_tables/ES1_rh}
%\include{_tables/VaR5_rh}
%\include{_tables/VaR1_rh}
%\include{_tables/var_rh}
