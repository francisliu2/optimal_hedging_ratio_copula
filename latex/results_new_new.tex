...
First, we draw a block of samples $\{X_i, ..., X_{i+j-1}\}$, where the index $i$ is a
...
Then, we repeat the procedure until the number of samples are drawn in total across the blocks equal or exceed a predefined number $S$. 
One bootstrapped time series sample is then the concantenation of the blocks truncated to length of $S$.
The bootstrapped time series is known as the pseudo time series. 

% For each block, we calculate the hedging effectiveness as outlined above.
We choose $p=1/5$, implying the average block length is 5.
The length of each pseudo time series sample $S = 300$ is chosen to be aligned with the length of each training set.  
The length of each pseudo time series sample is also chosen to reasonably calculate ES and VaR.
