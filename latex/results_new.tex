...
%\input{copulaeselectiontable.tex}

\subsection{Overview}\label{sec:overview}

\natp{\em [Start with some general time series plots of the BTC and
  the Bitcoin futures. Readers will want to know how BTCF developed
  during the time period. Explain some of the events during that
  time. Also add some descriptive stuff, e.g.\ it'll be interesting to
  see a correlation heatmap of the cryptos and indices.]}
...
copula is not Frank or rotGumbel. \natp{\em [These are already
  conclusions. Start with results.]}
...
\natp{\em [Again, below are conclusions. I suggest to either continue
  presenting results and then drawing conclusions at the end, or
  otherwise explain 
  explain how you draw this conclusion from the information so far.]}
...
This relationship explains every OHRs in BTC-BTCF portfolio are close to one. \natp{\em [The point you are trying to make is that there is a static
  no-arbitrage relationship between spot and futures price. This
  enforces a hedge ratio of one (at expiry you receive one unit of
  spot and deliver one unit of the future or vice versa). Hence, the
  log-returns between spot and futures will also be close, and taking
  into account spot financing the position, the log-return of the
  portfolio (spot, futures, money market account) will be zero. In
  cryptocurrencies we do not observe this relationship, though, but a
  basis between futures and spot. Nonetheless, the hedge ratio will be
  close to one. Please re-formulate according to these lines.]}

\natp{\em [Please read the rest critically before
  Thursday, so we can discuss. Consider the order of results: first
  the copula results, then the rest, because they build on each
  other. Think carefully about what you want to tell the reader.]}


\natp{\em [Please try to reduce the size of the figures so that they
  appear in the text.]}
...
%    \includegraphics[width=0.9\paperwidth]{_pics/overview.pdf}
  ...
  \caption{An overview of out-of-sample hedged portfolios.
            ...
            \natp{\em [I suggest to defer the remainder to the
              text.]} One exception is the BTC-BTCF portfolio that minimizes ES 99\% (first panel green line in middle plot), which fluctuates greatly from Jun 2019 to Aug 2019.
...
we illustrate the copula selection result in this section. \natp{\em
  [Please revise carefully. The previous sentence can be improved {\bf
    a lot}! Give proper information (Why does this help to understand
  the market better? Because the copulas were selected to model
  different features of the dependence...).]}
