...
\natp{\em [I find it tricky that we give Sklar's Theorem, but no
  definition of a copula. This should be added.]}
...
\begin{proposition}
  ...
  and $F_{Y}$. Then, the distribution function of $Z$ \natp{\em [What
    is $Z$?]} is given by 
  \begin{equation}
    \label{eq:3}
...
function. \natp{\em [Please check, but this holds only for $h>0$.]}
\end{proposition}
...
\begin{corollary} Given the formulation of random variables \natp{\em
    [What exactly does this mean?]}, the
  pdf of $Z$ can be written as
  \natp{\em [Since $h>0$, it is sufficient to write $1/h$. Order of
    arguments in $c_{X,Y}$ must be flipped.]}
  \begin{align}
  ...
  \end{align}
  \end{corollary}
The two expressions are equivalent. \natp{\em [This is obvious as
  their LHS's agree... I think you want to say something else. How is
  the second expression obtained?]}
...
densities.
A generic expression \natp{\em [Of what?]} and proof can be found in the
appendix. \natp{\em [Reference appendix properly.]}

\subsection{Procedure to determine optimal hedge ratio}\label{sec:empirical-procedure}
We introduce the empirical procedure to obtain the optimal hedge ratio
being used in this work. \natp{\em [This is really the
  backtesting procedure. I think it'd be good if the word ``backtest''
  appeared somwewhere.]}
...
Next, we obtain the optimal hedge ratio as follows:
...
\item \textbf{Determine optimal hedge ratio}.
  We determine the optimal hedge ratios numerically using different
  risk measures as 
  ...
  Finally, we apply the optimal hedge ratios to the test data $R_h =
  R_s - h^* R_f$. 
  \end{enumerate}
